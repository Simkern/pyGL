...
def upotest():
    ...
    print(f"x0(0): {x0.T}")
    print(f"T: {t0}")
    print(f"f(x0(0)): {rossler(0, x0, p).T}")
    xT = Φ(x0, t0, p)(t0)
    print(f'params = {a}, {b}, {c}')
    print(f"x0(T): {xT.T}")
    print(f"f(x0(0)): {rossler(0, xT, p).T}")
    x0,t0 = newton_upo(x0, t0, p, maxiter=100, tol=1e-10)
    print(f'period: {t0}')
    A = np.array([dΦ(np.eye(3)[:, i], x0, t0, p) for i in range(3)])
    print(A)
    λ = np.linalg.eigvals(A)
    print(f"Eigenvalues : {λ}")
    return x0, t0
...
if __name__ == "__main__":
    ...
    J = np.block([[A.T, b], [c.T, np.array([0])]])
    
    #rd = np.random.rand(3,)
    #x0 = np.array([ np.round(r,3) for r in rd ])
    x0 =  np.array([1,1,0])
