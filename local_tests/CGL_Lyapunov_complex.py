...
#p_v = np.array(p_v[0], ndmin=1, copy=False)
#pin[0] = np.real(pin[0])
#pin[3] = np.real(pin[3])
...
for nAinv in range(0,n+step,step):
    ...
    pin = np.random.permutation(np.concatenate((pA_v[:nA],pAinv_v[:nAinv])))
    Z, ires, res, res_rel, nrmx, nrmz, nrmz_rel = lrcfadic_r(A, B, pin, tol,'tol', Xref)
    #Z, ires, res, res_rel, nrmx, nrmz, nrmz_rel = lrcfadi(A, B, pin, niter,'niter',Xref)
    plt.plot(ires,np.log10(nrmx),label=f'n = {n}, pA,pAinv = ({nA},{nAinv})')
